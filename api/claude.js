// api/claude.js — Claude API 프록시 + MHT 파싱
// Base64 인코딩된 MHT 파일을 서버에서 파싱 후 Claude API 분석
...
// MHT 파싱 — Base64 및 quoted-printable 모두 처리
function parseMHT(mhtText) {
  ...
  for (const part of parts) {
    ...
    encoding       = encMatch?.[1]?.trim().toLowerCase() || 'none';
  ...
  if (!htmlPart) {
    ...
    htmlPart = bodyMatch?.[1]?.trim() || mhtText;
